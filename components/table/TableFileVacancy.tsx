...
export default function TableFileVacancy({ data, nameModule, id }: Props) {
    // Snackbar State
    ...
    const columns: GridColDef[] = [
        ...
        },
        {
            field: 'size', headerName: "Size", width: 80, align: "left", renderCell: (params) => (
                <>
                    <p>
                        {(params.row.size / 1024).toFixed(3)} KB
                    </p>
                </>
            )
        },
        {
            field: 'action', headerName: "", width: 120, headerAlign: "center", align: "center", renderCell: (params) => (
                <>
                    <button className="bg-[#3b82f6cf] rounded-lg text-white p-2 m-1 hover:bg-[#466698cf] duration-500" onClick={() => handleDownload(params.row.fileName)}>
                        <FileDownloadRoundedIcon />
                    </button>
                    <button className="bg-[#C60000] rounded-lg text-white p-2 m-1 hover:bg-[#911212] duration-500" onClick={() => handleOpenAlertDelete(params.row.fileName)}>
